...
div.account
	...
	Row(:gutter=50 style="padding-bottom:15%")
		Col(span=12)
			...
			h3.account-title(style="text-align:center;font-size:23px;line-height:50px;width:250px;color:#90fc30;" v-if="this.$i18n.locale == 'en-US'")|{{$t("NBSL")}}
			h3.account-title(style="text-align:center;font-size:32px;color:#90fc30;font-family:'楷体'" v-if="this.$i18n.locale == 'zh-CN'")|{{$t("NBSL")}}
				...
				Form.account-form(ref='form',:model="form",:rules="rules",:label-width="80")
					...
					Form-item
						Row(:gutter="20")
							...
							Col(span=12)
								div(style="cursor: pointer;text-align:center;color:#90fc30;" @click="reset")|{{$t("forget_password?")}}
							Col(span=12 align="center")
								checkbox(v-model="rem" @click="rem=!rem" style="color:#90fc30;")|{{$t("remember")}}
							Col(span=5)
								Button(type="text",@click="changelang" style="color:#90fc30;")|<
							Col(span=14)
								div(style="width:100%;text-align: center;color:#90fc30;")|{{$t('lang')}}
							Col(span=1)
								Button(type="text",@click="changelang" style="color:#90fc30;")|>
</template>
